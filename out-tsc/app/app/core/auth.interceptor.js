...
export const authInterceptor = (request, next) => {
    ...
    return next(authorized).pipe(catchError((error) => {
        ...
        return auth.refresh().pipe(switchMap(tokens => next(request.clone({ setHeaders: { Authorization: `Bearer ${tokens.accessToken}` } }))), catchError(refreshError => { auth.clear(); return throwError(() => refreshError); }));
    }));
};
